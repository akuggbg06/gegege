...
export async function getAllMedia(username) {
  const images = await getCollection('images');
  const videos = await getCollection('videos');
  ...
  const allMedia = [...userImages, ...userVideos].sort((a, b) => 
...
export async function getPublicMedia() {
  ...
  const allPublic = [...publicImages, ...publicVideos].sort((a, b) => 
    new Date(b.uploaded_at) - new Date(a.uploaded_at)
  );
...
export async function deleteMedia(username, mediaId, type) {
  ...
  // Hapus dari Telegram (opsional, biar bersih)
